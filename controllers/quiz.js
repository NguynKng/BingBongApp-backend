  ...
  try {
    ...
    if (name && name.trim() !== "") {
      ...
      const filteredQuizzes = quizzes.filter((quiz) => {
        const normalizedQuizName = removeDiacritics(quiz.title.toLowerCase());
        return normalizedQuizName.includes(normalizedInput);
      });

      return res.json({ success: true, quizzes: filteredQuizzes });
    }
...
const deleteQuiz = async (req, res) => {
  try {
    ...
    if (quiz.createdBy.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ success: false, message: "Unauthorized to delete this quiz" });
    }
