  ...
  try {
    ...
    if (existing) {
      if (existing.type === type) {
        ...
        return res
          .status(200)
          .json({ message: "Reaction removed", success: true });
      } else {
        ...
        return res
          .status(200)
          .json({ message: "Reaction updated", success: true, data: existing });
      }
    }
    ...
    await post.save();
    const populatedReaction = await newReaction.populate(
      "user",
    ...
    if (post.author.toString() !== userId.toString()) {
      ...
      await sendNotification([post.author], userId, "react_post", {
        postId: post._id,
